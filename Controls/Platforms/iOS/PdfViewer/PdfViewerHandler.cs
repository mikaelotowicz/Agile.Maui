    ...
    private void StartRenderWindow(int centerPage, int start, int end)
    {
        ...
        _ = Task.Run(async () =>
        {
            foreach (int idx in order)
            {
                ...
                if (cache.Contains(idx)) { MainThread.BeginInvokeOnMainThread(() => pv?.EnsurePageView(idx)); continue; }
    ...
    public void EnsurePageView(int idx)
    {
        ...
        if (!_pageViews.TryGetValue(idx, out var iv))
        {
            iv = new UIImageView
            {
                ...
                Frame           = new CGRect(0, _pageOffsets[idx], _contentView.Frame.Width, _pageHeights[idx]),
            };
